...
//mybonescriptspack
function MakeBonesFlyUp(Length, Type){
		var cnm = 0
		var rand = 220;
		
		repeat (20)
    {
        var bone = Battle_MakeBone(rand + (cnm * 25), 215, 20, 0, 0, Type, 0, 90, 0, 1);
        bone.length = Length;
        Anim_Create(bone, "y", 2, 1, 220, 20, 15, 0);
        Anim_Create(bone, "y", 2, 1, 240, -20, 15, 15);
        Anim_Create(bone, "vspeed", 2, 1, 0, 4, 1, 30);
        cnm += 1;
    }
	return bone
}
